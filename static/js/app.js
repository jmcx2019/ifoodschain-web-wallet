...
app.controller("WalletCtrl", function($scope, $translate, $http, $sce, $interval, $modal, $filter) {
  ...
  $scope.init = function () {
     ...
     * 加载node配置:
     */
    $http.get('static/conf/wallet-conf.json?20180207').then(function (data) {
    ...
    Wallet.SendTransactionData($http, $txData, host, (function (res) {
      if (res.status == 200) {
        ...
        if (res.data.Error == 0) {
          ...
          successInfo = successInfo + ab2hexstring(txhash)
